(function () {
    ...
    .service('parentService', function ($scope, $http, configService) {
        $scope.parent = [];

        $scope.fetchParent = function () {
            $http.get(configService.siteRoot + 'api/read/parent', {
                params: {
                    'courseId': configService.courseId
                }
            }).then(function (response) {
    ...
    .service('courseService', function ($scope, $http) {
        ...
        //  Fetches and sets courseChild (parameters hunder particular child)
